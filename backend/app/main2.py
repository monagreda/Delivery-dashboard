...
from backend.app.auth import verify_password, create_access_token, get_password_hash, SECRET_KEY, ALGORITHM
from backend.app.models import UserRegister, OrderAssignment, OrderCreate
...
@app.post("/orders")
async def create_order(order_data: OrderCreate, current_user = Depends(get_current_user), conn=Depends(get_db)):
    # Cambiamos el prefijo para la empresa de UK
    ...
    with conn:
        with conn.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO orders (
                    order_id, lng, lat, user_id, status, 
                    origin_country, origin_address, origin_postcode,
                    destination_country, destination_address, destination_postcode,
                    weight_kg, volume_m3, cargo_description
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """,
                (
                    order_id, order_data.lng, order_data.lat, current_user["id"], "pending",
                    order_data.origin_country, order_data.origin_address, order_data.origin_postcode,
                    order_data.destination_country, order_data.destination_address, order_data.destination_postcode,
                    order_data.weight_kg, order_data.volume_m3, order_data.cargo_description
                )
            )
